...
import { TypingSpeedFeature } from './typingSpeed';

let historyPanel: vscode.WebviewPanel | undefined;

export function showHistoryPanel(context: vscode.ExtensionContext, history: any[]) {
    // If we already have a history panel, show it instead of creating a new one
    if (historyPanel) {
    ...
    // Handle messages from the history webview
...
export function getHistoryWebviewContent(webview: vscode.Webview, context: vscode.ExtensionContext, history: any[]): string {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <script>
        ...
        function filterDataByTimeRange(history, range) {
            ...
            switch(range) {
                case 'last-hour':
                    cutoffTime = now - (60 * 60 * 1000); // 1 hour in milliseconds
                    break;
                case 'last-day':
                    cutoffTime = now - (24 * 60 * 60 * 1000); // 24 hours in milliseconds
                    break;
                case 'last-week':
                    cutoffTime = now - (7 * 24 * 60 * 60 * 1000); // 7 days in milliseconds
                    break;
                case 'all-time':
                default:
        ...
        function sampleDataForDisplay(history, maxPoints = 100) {
            ...
            // For longer histories, sample points to avoid overcrowding the chart
        ...
        function updateStats(filteredData) {
            ...
            // Calculate peak WPM
            const peakWpm = Math.max(...filteredData.map(entry => entry.wpm));
        ...
        function updateChart(history, range) {
            ...
            // Update chart title to reflect data range
            let title = 'WPM over ';
            switch(range) {
